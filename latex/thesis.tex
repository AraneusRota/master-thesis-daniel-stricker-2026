...
\chapter{Proof of Deadlock-Freedom}
An open task of [ChorLean] is to show that it is free of deadlocks. They already sketched a proof but a full proof would be nice. [There are multiple possibilities.]

\section{cgraphs}
[Thiemann] suggested that it could be proven using [interaction trees]. We investigated this and found [cgraphs] which sounded promising. Unfortunately it is written in Coq which we are not exactly familiar with. Furthermore, it turns out that a simple example proof using [cgraphs] is 1500 lines of code long and consists of \emph{very} dense code. [Other] possibilities turned out to have similar problems.


\chapter{ChorLean}
\section{Group Chat (dynamic roles)}
t
\section{Exceptions}
e
