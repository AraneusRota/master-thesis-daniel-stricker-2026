...
\abstract{In this work we explore programming languages and their features in the context of distributed programming. We first explore session types which promise to proof correct communication between peers. Another paradigm which promises a similar feat is choreographic programming. We inspect ChorLean, a \chorlang{} which uses a dependent type system to add even more guarantees than other \chorlangs{}. We try to extend it with more features and trying to prove its deadlock freedom. We explore the fundamental differences between \chorlangs{} and multitier programming. Furthermore we look into which alternative \chorlangs{} exist and we find an interesting language \lambdax{} which promises to build a common ground for all state of the art \chorlangs{}. The only missing piece: A mechanization, which we try to implement and to even extend it using a dependent type system.}
...
We will implement Thiemann's\cite{thiemann2023intrinsicallytypedsessionscallbacks} session types using Lean to possibly combine them with the existing \chorlang{} ChorLean:
\begin{lstlisting}
inductive Type'
  | int : Type'
  | bool : Type'
...
open Session
\end{lstlisting}
First we have to define some primitive data types for our language to be based on (lines 1--3). The main part is the inductive \lstinline{Session} definition. With session types we want to define a protocal of communication which is statically always fulfilled. One could think about which communication primitives exist. The initial logical choice is send and receive.

We will start with exactly them, \lstinline{_!} (send) in line 6 and \lstinline{_?} (receive) in line 7. They have two parameters each. First the type of the data to be sent/received and another session which acts as a continuation. Thiemann's main achievement is to abuse continuations. The thing is that for session types you have to guarantee that every session is only accessed once which can also be called a linear type system. Designing session types with a linear type system is hard and some languages don't even support linear type systems. Turns out continuations fullfill the same goal and are more often supported than linear type systems.

This is also called continuation passing style and means that after every operation we have to pass the next opeartion the function we called. The function usually does not return normally but instead calls the continuation with its result.

To end this unending cycle we introduce \lstinline{end'} (line 8) which is used as a continuation when we do not want futher operations to happen.

The problem is we cannot change flow of our program yet we can only statically send or receive as often as we want. Therefore, we add \lstinline{internalChoice} (line 9) and \lstinline{externalChoice} (line 10). \lstinline{internalChoice} means that we do some computation which produces a number of type \lstinline{Fin k} which we then send to our communication partner. He then executes the expression with the \lstinline{Session} type that is returned by the function which is passed to its \lstinline{externalChoice} counterpart. 

Let us look at a small example:
\begin{lstlisting}
def server := _? int $ _? int $ _! int end'
def client := dual server

def equivalentClient := _! int $ _! int $ _? int end'

\end{lstlisting}
\lstinline{binary} is a session type which represents a server which receives an \lstinline{int} and then another \lstinline{int}. Then it (no necessarily) does some operation on those two arguments and sends the result back. Then the session ends.

A direct communication parter's session can easily be generated automatically with a \lstinline{dual} function which just recursively flips sends/receives and \lstinline{internalChoice}/\lstinline{externalChoice}:
...
def dual
    ...
    | _? t s => _! t $ dual s
    | end' => end'
    | internalChoice sessionSelector => 
        externalChoice (fun label => dual $ sessionSelector label)
    | externalChoice sessionSelector => 
        internalChoice (fun label => dual $ sessionSelector label)
\end{lstlisting}

We also added loops and recursion which makes everything more complicate. Unfortunately we could not reach our goal of combining session types with ChorLean. ChorLean's viewpoint of its distributed code is just too different. ChorLean takes a global viewpoint from the top where we only need a \lstinline{com} primitive at one place which handles sending and receiving at the same time. 

Session types are fundamentally built to model one side of a communication. It is possible to generate the other side by using functions like \lstinline{dual} but fundamentally a session type only represents one side which is not compatible with \chorlangs.
...
\section{Lean and dependent types}
...
Getting deep into the implementation of ChorLean is out of scope for this work, but we will showcase the central type \lstinline{Choreo} because looking at its declaration already gives us deep insight into how some unique features of \lstinline{ChorLean} work. Here is the declaration (code from ChorLean\cite{chorleanrepo}):
...
Klor is a \chorlang{} implemented in Clojure. Clojure is part of a bigger family of programming languages called Lisps\cite{dagand2025lisp}. One could call Lisps the antithesis to pure functional programming languages with dependent types like Lean.

The first lisp was developed by John McCarthy in 1960\cite{10.1145/367177.367199} which started a family of similar languages but the concepts are usually the same. The main premise of Lisp is that every program can be represented by a very simple syntax. Every program and therefore every expression is just a simple linked list with symbols as data in each cell. At first glance a more simple syntax seems nonessential and can even seem unreadable for the untrained eye because of many explicit parentheses.
...
(defchor rps-start [A B] (-> #{A B}) []
  (let [a (A (string->hand (read-line)))
    ...
    (A (prn "Winner:" winner))))
\end{lstlisting}
We define a choroegraphy by calling \lstinline{defchor} (line 1) and providing the types. We say that \lstinline{A} and \lstinline{B} are possible communication partners. In line 1 we also define that this choreography returns a value which is located at both A and B denoted by the \lstinline{#} with curly braces which denote a set in Clojure. Note that the location is typed (and statically checked by a macro) but the actual value stays dynamic because Clojure is dynamic. 

In lines 2 we read the input from the console of \lstinline{A}, convert the string into a hand enum and assign it to variable \lstinline{a}. The \lstinline{A} in this line is necessary to tell the compiler where to execute the functions, in this case at \lstinline{A}. \lstinline{a} is therefore only located at \lstinline{A}. This is analogous for line 3 and \lstinline{B}.

The interesting part about line 4 is \lstinline{(B->A b)}. There we can see the interesting syntax sugar a macro implementation can give us. The macro detects that \lstinline{A} and \lstinline{B} are possible roles and allows this syntax with a arrow to send a value from one peer to another. After printing the result (here only at \lstinline{A}) we are finished. 

\section{Mechanization of \lambdax{}}
...
A key insight of \lambdax{} is the semantics which all \chorlangs{} follow. That is, they are neither strict nor lazy. According to Plyukhin et al.\cite{10.1145/3747538} \lambdax{} this seems to be the main reason why no one could find a simple core set of rules for a choreographic lambda calculus. Chor$\lambda$ had found a set of working rules but \lambdax{} could reduce the number significantly.

A major feat of \lambdax{} is its simplicity. By developing the language from the ground up with a fresh perspective and using different abstractions \lambdax{} found out that multiple rules used by Chor$\lambda$\cite{cruz-filipe2022functional} were in fact unnecessary because they could be replaced by other rules. They even found out that Chor$\lambda$ was missing some rules. They are very specific and rare cases but nonetheless it is a sign that \lambdax{} uses a very structured and sensible approach if they can find that so easily.
...
First we have to define some auxiliary types. That is \lstinline{HList} where the ``H'' stands for homogenous. This is an example taken from the official Lean language website\cite{hlist}:
\begin{lstlisting}[mathescape]
...
\section{MultiChor}
\lstinline{MultiChor}\cite{bates2024multichorcensuspolymorphicchoreographic} is a \chorlang{} implemented with Haskell, another purel functional programming language. Let us look at this example:
\begin{lstlisting}
...
choreography = do
  ...
  locally_ alpha \un -> putOutput "Winner:" $ winner
  locally_ beta \un -> putOutput "Winner:" $ winner
\end{lstlisting}
In lines 7--8 we read the input of peer \lstinline{alpha}, save it in \lstinline{a} and share it to \lstinline{beta} and saving it in \lstinline{a'}.

In lines 12--15, \lstinline{alpha} locally decides the winner. Note that we have to unpack the values with \lstinline{un} here like in ChorLean. Then we just print the winner locally each at \lstinline{alpha} and \lstinline{beta}.

\section{ChoRus}
We also have an implementation with \lstinline{ChoRus}\cite{kashiwa2023portableefficientpracticallibrarylevel} using Rust but we will leave out further explanation because nothing particularly interesting happens here: 
...
We compared multiple \chorlangs{} and tried to improve them by combining knowledge of them. We cared about seeing multiple distinct views of every problem and tried to combine the knowledge of seemingly distinct implementations/research. \lambdax{}\cite{10.1145/3747538} also looked at multiple existing \chorlangs to improve them by designing a common denominator.

We compared multiple distinct paradigms like \chorlangs{}, multitier programming and session types. Giallorenzo et al.\cite{giallorenzo2021multiparty} worked specifically on the pair of multitier programming and \chorlangs{}.

By focusing on using Lean to implement most of our code, for example \lambdax{} we tried to have more secure systems with better static guarantees. For example Thiemann\cite{thiemann2023intrinsicallytypedsessionscallbacks} does focus on provable communication guarantees by researching session types on a theoretical level.  
