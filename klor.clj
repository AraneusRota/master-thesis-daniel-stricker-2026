...
(def select-winner {[:rock :paper]   :left
                    [:rock :scissors]     :right
                    [:rock :rock]         :draw
                    [:paper :rock]        :right
...
(def string->hand {"rock" :rock 
                   "paper" :paper
                   "scissors" :scissors})

(defchor rps-start [A B] (-> #{A B}) []
  (let [a (A (string->hand (read-line)))
        b (B (string->hand (read-line)))
        winner (A (select-winner [a (B->A b)]))]
    (A (prn "Winner:" winner))))
...
(comment
  (def rps-server
    (future
      (with-server [ssc {:port rps-port}]
                   (loop []
                     (println "Listening on" (str (.getLocalAddress ssc)))
                     (with-accept [ssc sc]
                                  (println "Got client" (str (.getRemoteAddress sc)))
                                  (play-role (wrap-sockets {:role 'A} {'B sc} :log true) rps-start))
                     (recur))))))
    
(with-client [sc {:host ip :port rps-port}]
             (println "Connected to" (str (.getRemoteAddress sc)))
             (play-role (wrap-sockets {:role 'B} {'A sc} :log true) rps-start))